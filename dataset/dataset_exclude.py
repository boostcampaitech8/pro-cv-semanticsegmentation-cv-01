...
class XRayDataset(Dataset):
    def __init__(self, is_train=True, transforms=None):
        ...
        # =============================================================
        # [Direct Filtering] 이번 실험을 위한 직접 필터링
        # =============================================================
        # EDA에서 발견한 문제 데이터 ID 목록 (반지, 네일아트, 노이즈 등)
        exclude_ids = ["ID363", "ID387"] 
        
        # 위 ID가 파일 경로에 포함되어 있으면 리스트에서 아예 제거
        pngs = {
            fname for fname in pngs 
            if not any(ex_id in fname for ex_id in exclude_ids)
        }
        
        print(f">> [Dataset] {len(exclude_ids)}개의 Artifact ID 제외 완료. 남은 데이터: {len(pngs)}장")
        # =============================================================
