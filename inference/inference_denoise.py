...
# [추가] 작은 노이즈(먼지) 제거 함수
def remove_small_noise(mask, threshold=900) :
    """
    mask: (2048, 2048) binary numpy array
    threshold: 지울 픽셀 수 - 기본 900
    """
...
def test():
        ...
        for images, image_names in tqdm(test_loader, desc="Inference"):
            ...
            for output, image_path in zip(outputs, image_names):
                ...
                for c, segm in enumerate(output):
                    ...
                    # 밑 2줄에서 처리 
                    threshold = 900
                    clean_segm = remove_small_noise(segm, threshold)

                    rle = encode_mask_to_rle(clean_segm)
    ...
    if os.path.exists(sample_sub_path):
        ...
        save_path = f"test_submission_{Config.EXPERIMENT_NAME}.csv"
