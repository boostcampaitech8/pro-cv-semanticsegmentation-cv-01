...
def predict_one_image(model, image, tta_mode=None, tta_scales=None, **kwargs):
    ...
    for scale in tta_scales:
        ...
        for t_name, forward_aug, backward_aug in transforms_list:
            ...
            with torch.amp.autocast(device_type="cuda"):
                # Check for Sliding Window Config
                window_size = getattr(Config, 'WINDOW_SIZE', None)
                stride = getattr(Config, 'STRIDE', None)
                
                if window_size is not None and getattr(Config, 'USE_SLIDING_WINDOW', True):
                     # SLIDING WINDOW MODE
                     if getattr(Config, 'USE_GAUSSIAN_SLIDING', True): # Default to Gaussian
                         try:
                             from inference.inference_sliding_GaussianOptThr import predict_sliding_image
                         except ImportError:
                             from inference_sliding_GaussianOptThr import predict_sliding_image
                     else:
                         try:
                             from inference.inference_sliding import predict_sliding_image
                         except ImportError:
                             from inference_sliding import predict_sliding_image
                     
                     # Call Sliding Window
                     if stride is None: stride = window_size // 2
                     
                     # predict_sliding_image returns PROBABILITIES (Sigmoid applied)
                     out = predict_sliding_image(model, aug_input.squeeze(0), window_size, stride)
                     out = out.unsqueeze(0) # (1, C, H, W)
                else:
                    # Standard Forward
                    out = model(aug_input)
                    if isinstance(out, dict): out = out['out']
                    # Apply Sigmoid here to Average PROBABILITIES (consistent with Sliding)
    ...
    # Average PROBS (CPU)
    avg_probs = ensemble_pred_cpu / count
    
    # Canonical Size (2048)
    if avg_probs.shape[2] != 2048:
        avg_probs = F.interpolate(avg_probs, size=(2048, 2048), mode="bilinear", align_corners=False)
        
    return avg_probs.squeeze(0) # Returns PROBABILITIES (C, 2048, 2048)
        ...
        for images, image_names in tqdm(loader, desc="Inference"):
            ...
            for img, name in zip(images, image_names):
                ...
                # 3. Default RLE Return (If not saving)
                    ...
                    # Import Optimal Thresholds (From Config)
                    optimal_thresholds = getattr(Config, 'OPTIMAL_THRESHOLDS', {})

                    # avg_probs is already Probabilities (0-1)
                    for c in range(avg_probs.shape[0]):
                        class_name = Config.CLASSES[c]
                        thr = 0.5
                        if getattr(Config, 'USE_OPTIMAL_THRESHOLD', True):
                            thr = optimal_thresholds.get(class_name, 0.5)
                        
                        segm = (avg_probs[c] > thr)
