...
def predict_sliding_image(model, image, window_size, stride):
    ...
    sigma = getattr(Config, 'GAUSSIAN_SIGMA', 0.5)
    gaussian_window = create_gaussian_window(window_size, sigma=sigma, device=image.device)
    
    # Debug Print 
    if not hasattr(predict_sliding_image, "_printed"):
        print(f"   [DEBUG] Gaussian Sliding Window Applied! (Sigma={sigma})")
        predict_sliding_image._printed = True
